...
public class VitalServiceSql extends AbstractVitalServiceImplementation {

	VitalSqlImplementation impl;
	
	public static VitalSqlConfig toInnerConfig(VitalServiceSqlConfig config) {
		
		VitalSqlConfig innerConfig = new VitalSqlConfig();
		
		innerConfig.setDbType(SqlDBType.valueOf(config.getDbType().name()));
		innerConfig.setEndpointURL(config.getEndpointURL());
		innerConfig.setPassword(config.getPassword());
		innerConfig.setPoolInitialSize(config.getPoolInitialSize());
		innerConfig.setPoolMaxTotal(config.getPoolMaxTotal());
		innerConfig.setTablesPrefix(config.getTablesPrefix());
		innerConfig.setUsername(config.getUsername());
		return innerConfig;
	}
	
	public VitalServiceSql(VitalServiceSqlConfig config,
			VitalOrganization organization, VitalApp app) {
		super(new SystemSegment(new VitalSqlSystemSegmentExecutor(new VitalSqlImplementation(new VitalSqlDataSource(toInnerConfig(config))))), organization, app);
	...
	@Override
	protected VitalStatus _deleteList(VitalTransaction transaction, List<URIProperty> uris)
			throws VitalServiceException, VitalServiceUnimplementedException {
		try {
	...
	@Override
	protected ResultList _saveList(VitalTransaction transaction, VitalSegment targetSegment,
	...
	@Override
	protected ResultList _callFunction(String function,
	...
	@Override
	protected ResultList _selectQuery(VitalSelectQuery query)
			throws VitalServiceException, VitalServiceUnimplementedException {
		try {
	...
	@Override
	protected ResultList _graphQuery(VitalGraphQuery query)
			throws VitalServiceUnimplementedException, VitalServiceException {
		try {
	...
	@Override
	protected VitalStatus _bulkImport(VitalSegment segment,
	...
	@Override
	protected VitalStatus _bulkExport(VitalSegment segment,
	...
	@Override
	protected void _rollbackTransaction(TransactionWrapper transactionWrapper) throws VitalServiceException {
