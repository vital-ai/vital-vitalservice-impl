...
public abstract class LuceneVitalServiceAdminImplementation2 extends AbstractVitalServiceAdminImplementation {
	...
	protected LuceneVitalServiceAdminImplementation2(
			VitalOrganization _organization, LuceneServiceImpl serviceImpl) {
		super(new SystemSegment(new LuceneSystemSegmentExecutor(serviceImpl)), _organization);
	...
	protected VITAL_GraphContainerObject _getExistingObjects(VitalApp app, List<String> uris) {
		try {
			return serviceImpl.getExistingObjects(listSegments(app), uris);
		} catch (Exception e) {
			throw new RuntimeException(e);
		}
	}
	...
	@Override
	protected VitalStatus _deleteAll(VitalApp app, VitalSegment segment) throws VitalServiceException, VitalServiceUnimplementedException {
		try {
