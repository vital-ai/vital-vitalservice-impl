...
public class VitalServiceAdminIndexedDB extends AbstractVitalServiceAdminImplementation {
	...
	private final static Logger log = LoggerFactory.getLogger(VitalServiceAdminIndexedDB.class);
	
	private VitalServiceAdminIndexedDB(IndexInterface index, DBInterface database, VitalOrganization _organization) {
		super(new SystemSegment(new IndexDBSystemSegmentExecutor(index.getSystemSegmentExecutor(), database.getSystemSegmentExecutor())), _organization);
		this.index = index;
...
//		if(organization == null) throw new RuntimeException("Organization not found: ${_organization.ID}")
	...
	public static VitalServiceAdminIndexedDB create(VitalServiceIndexedDBConfig config, VitalOrganization organization) {
		...
		VitalServiceAdminIndexedDB instance = new VitalServiceAdminIndexedDB(indexInterface, dbInterface, organization);
...
//				return VitalServiceIndexedDB.resolveResultsList(organization, app, database, rs, gq, listSegments);
	...
	public VitalStatus verifyIndices() throws Exception {
		...
		for(VitalOrganization c : Arrays.asList(organization)) {
			...
			for(VitalApp a : databaseApps) {
				...
				for(VitalSegment s : segs) {
					...
					if( indexExecutor.getSegmentResource(s) == null ) {
						...
						errors.add("Segment exists in database but not in index - appID: " + appID + ", segmentID: " + segmentID + " segmentURI: " + s.getURI());
					...
					if(indexSize != databaseSize) {
						...
						errors.add("Different segment index size (" + indexSize + ") and database size (" + databaseSize + "): appID: " + appID + ", segmentID: " + segmentID + " segment URI: " + s.getURI());
	...
	public VitalStatus verifyIndex(VitalApp a, VitalSegment s) throws Exception {
		...
		ResultList segmentsRL = systemSegment.listSegments(organization, a);
		
		if( executor.getDbExecutor().getSegmentResource(s) == null ) {
			return VitalStatus.withError("Segment not found - app: " + appID + " segmentID: " + segmentID + " segmentURI: " + s.getURI());
		}
		...
		if( indexExecutor.getSegmentResource(s) == null ) {
			...
			return VitalStatus.withError("Segment exists in database but not in index - appID: " + appID + ", segmentID: " + segmentID + " segmentURI: " + s.getURI());
		...
		if(indexSize != databaseSize) {
			...
			return VitalStatus.withError("Different segment index size (" + indexSize + ") and database size (" + databaseSize + "): appID: " + appID + ", segmentID: " + segmentID + " segment URI: " + s.getURI());
...
//		List<VitalOrganization> databaseOrganizations = systemSegment.listOrganizations();
		...
		for(final VitalOrganization c : Arrays.asList(organization)) {
	...
	protected void _commitTransaction(TransactionWrapper transaction) throws VitalServiceException {
		...
		ResultList listSegments = systemSegment.listAllOrganizationSegments(organization);
	...
	@Override
	protected VITAL_GraphContainerObject _getExistingObjects(VitalApp app,
	...
	@Override
	public VitalStatus _bulkExport(VitalApp app, VitalSegment segment, OutputStream outputStream, String datasetURI)
	...
	@Override
	public VitalStatus _bulkImport(VitalApp app, VitalSegment segment, InputStream inputStream, String datasetURI)
		throws VitalServiceUnimplementedException, VitalServiceException {
		ResultList listSegments = systemSegment.listSegments(organization, app);
		try {
